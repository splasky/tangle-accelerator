...
extern struct device_type wp76xx_device_type;

static le_sim_Id_t SimId;
/* UART file descriptor */
static int uart_fd;

static status_t wp76xx_init(void) {
  status_t err = register_device(&wp76xx_device_type);
  LE_ERROR_IF(err != SC_OK, "register wp76xx device error: %d", err);
  return err;
}

static void wp76xx_release(void) { LE_INFO("Finalize the device success"); }

static status_t wp76xx_get_key(uint8_t *key) {
...
static status_t wp76xx_get_device_id(char *device_id) {
...
static const struct device_operations wp76xx_ops = {
    .init = wp76xx_init,
    .fini = wp76xx_release,
    .get_key = wp76xx_get_key,
    .get_device_id = wp76xx_get_device_id,
};

static const struct uart_operations wp76xx_uart = {
    .init = uart_init,
    .write = uart_write,
    .read = uart_read,
    .clean = uart_clean,
...
static const struct secure_store_operations wp76xx_sec_ops = {
    .init = sec_init,
    .write = sec_write,
    .read = sec_read,
    .delete = sec_delete,
...
struct device_type wp76xx_device_type = {
    .name = "wp76xx",
    .op = &wp76xx_ops,
    .uart = &wp76xx_uart,
    .sec_ops = &wp76xx_sec_ops,
};

DECLARE_DEVICE(wp76xx);
